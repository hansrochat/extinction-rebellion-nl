          ...
          <div class="join-action-btns">
            <?php if (is_array($section->broadcasts)) : ?>
              <?php foreach ($section->broadcasts as $broadcast) : ?>
                <a class="btn btn-pink my-2" href="<?php echo ($broadcast['link']); ?>" onclick="<?= register_button_click($broadcast['label'] . ' (broadcast)'); ?>"><?php echo ($broadcast['label']); ?></a>
              <?php endforeach; ?>
            <?php endif; ?>
          </div>
        </div>
          ...
          <div class="join-action-btns">
            <?php if (is_array($section->groups)) : ?>
              <?php foreach ($section->groups as $group) : ?>
                <a class="btn btn-yellow my-2" href="<?php echo ($group['button_link']); ?>" onclick="<?= register_button_click($group['button_label']); ?>"><?php echo ($group['button_label']); ?></a>
              <?php endforeach; ?>
            <?php endif; ?>
          </div>
        </div>
      ...
      <div class="row">
        <div class="col-12 col-sm-10 col-md-8 col-lg-6 col-xl-6 mx-auto">
          ...
          <?php if (!empty($section->button_label)) : ?>
            <a class="btn btn-black" href="<?php insertURL(7221) ?>" onclick="<?= register_button_click($section->button_label); ?>"><?php echo ($section->button_label); ?></a>
          <?php endif; ?>
        </div>
      </div>
    </section>
    ...
    <section class="join-section bg-blue text-white join-cover-image" style="background: linear-gradient(rgba(0, 0, 0, 0.65), rgba(0, 0, 0, 0.45)), url('<?php echo ($section->cover_image); ?>') no-repeat;">
      <div class="row">
        <div class="col-12 col-sm-10 col-md-8 col-lg-6 col-xl-6 mx-auto">
          ...
          <p><?php echo ($section->description); ?></p>
          <?php if (!empty($section->button_label)) : ?>
            <a class="btn btn-blue" href="<?php insertURL(548) ?>" onclick="<?= register_button_click($section->button_label); ?>"><?php echo ($section->button_label); ?></a>
          <?php endif; ?>
        </div>
      </div>
    </section>
                  ...
                  <?php if (is_array($action['buttons'])) : ?>
                    <?php foreach ($action['buttons'] as $button) : ?>
                      <?php if ($button['button_label']) : ?>
                        <a class="btn btn-black my-2 mx-2" href="<?php echo ($button['button_link']); ?>" target="_blank" onclick="<?= register_button_click($button['button_label']); ?>"><?php echo ($button['button_label']); ?></a>
                      <?php endif; ?>
                    <?php endforeach; ?>
                  <?php endif; ?>
                </div>
